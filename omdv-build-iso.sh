...
getPkgList() {
	...
	if [ ! -d "$WORKDIR/iso-pkg-lists-${TREE,,}" ]; then
		...
		if [ ! -e "$FILELISTS" ]; then
        	...
        	echo "   Missing: omdv-$TYPE.lst file, check the $WORKDIR/iso-pkg-lists-$TREE/ folder"
        	errorCatch
		fi
	    else
		...
		if [ ! -e "$FILELISTS" ] || [ ! -e "$DISPLAYLISTS" ]; then
        	printf "%s\n" "-> Required file does not exist:"
        	[ ! -e "$FILELISTS" ] && echo "   Missing: omdv-$TYPE.lst file, check the $WORKDIR/iso-pkg-lists-$TREE/ folder"
        	[ ! -e "$DISPLAYLISTS" ] && echo "   Missing: omdv-$DISPLAYMANAGER.lst file, check the $WORKDIR/iso-pkg-lists-$TREE/ folder"
        	errorCatch
		fi
	    fi
	fi
